...
public class HttpClientUtil {
    ...
    private final static String RESP_CONTENT = CodeStatus.COMMUNICATION_FAILED_NETWORK.getName();
     ...
     * 集
     */
    public static String sendGetRequest(String reqURL, String param, Map<String, String> headers) {
        if (null != param) {
            reqURL += "?" + param;
        }
        String respContent = RESP_CONTENT; // 响应内容
    ...
    public static String sendPostRequest(String reqURL, String param, String type) {
        ...
        if (type != null && type.length() > 0) {
            ...
            httpPost.setHeader(HTTP.CONTENT_TYPE, "mu/form-data; charset=" + ENCODE_CHARSET);
        }
        CloseableHttpResponse response = null;
        try {
            if (param != null) {
            ...
            logger.info("开始执行请求：" + reqURL + "   params:" + param);
            // reqURL = URLDecoder.decode(reqURL, ENCODE_CHARSET);
    ...
    /**
     * 发送HTTP_POST请求 type: 默认是表单请求 headers: 请求头
     *
     * @param reqURL        请求地址
     * @param reqData       请求参数,若有多个参数则应拼接为param11=value11&22=value22&33=value33的形式
     * @param encodeCharset 编码字符集,编码请求数据时用之,此参数为必填项(不能为""或null)
     ...
     * @see 若响应消息头中未指定Content-Type属性,则会使用HttpClient内部默认的ISO-8859-1
     */
    public static String sendPostRequest(String reqURL, String param, String type, Map<String, String> headers) {
        String result = RESP_CONTENT;
        // 设置请求和传输超时时间
        HttpPost httpPost = new HttpPost(reqURL);
        // 这就有可能会导致服务端接收不到POST过去的参数,比如运行在Tomcat6.0.36中的Servlet,所以我们手工指定CONTENT_TYPE头消息
